...
@Service("com.jsoup.crawling.service.implementation.SearchEncryptCrawling")
@Slf4j
public class SearchEncryptCrawling {
    @Autowired
    ...
    private boolean checkIfLeadPresentInSearchEncrypt(Document document) {
        boolean isLeadPresent = true;
        if (null!=document) {

            try {
                Elements elements = document.getElementsByClass(SEARCHENCRYPT_RESULTS_LIST_CLASS);
                if (elements.size() == 0) {
                    isLeadPresent = false;
                }
...
//                if(checkIfLeadPresentInSearchEncrypt(document))
//                    break;
//                else{
...
//            }
    ...
    public JSONObject getSearchEncryptDataFromDocument(String searchQuery){
        ...
        if(null!=document) {
            try {
                Elements elementList = document.getElementsByClass(SEARCHENCRYPT_RESULTS_LIST_CLASS);
                if (CollectionUtils.isNotEmpty(elementList)) {
                    for (Element element : elementList) {
                        ...
                        String profileUrl = "";
                        try {
                            searchTitle = element.selectFirst(SPAN).text();
