...
@Service("com.jsoup.crawling.service.implementation.AolCrawling")
@Slf4j
public class AolCrawling {
    ...
    private boolean checkIfLeadPresentInAOL(Document document) {
        boolean isLeadPresent = true;
        if (null!=document) {

            try {
                Elements elements = document.getElementsByClass(AOL_RESULTS_LIST_CLASS);
                if (elements.size() == 0) {
                    isLeadPresent = false;
                }
...
//                if(checkIfLeadPresentInAOL(document))
//                    break;
//                else{
...
//            }
    ...
    public JSONObject getAOLDataFromDocument(String searchQuery){
        ...
        if(null!=document) {
            try {
                Elements elementList = document.getElementsByClass(AOL_RESULTS_LIST_CLASS);
                if (CollectionUtils.isNotEmpty(elementList)) {
                    for (Element element : elementList) {
                        ...
                        try {
                            ...
                            profileUrl = element.selectFirst(SPAN).text();
                        }
                        catch (Exception e){
