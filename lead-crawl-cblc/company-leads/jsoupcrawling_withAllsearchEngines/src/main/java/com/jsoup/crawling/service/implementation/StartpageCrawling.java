...
@Service("com.jsoup.crawling.service.implementation.StartpageCrawling")
@Slf4j
public class StartpageCrawling {
    @Autowired
    ...
    private boolean checkIfLeadPresentInStartpage(Document document) {
        boolean isLeadPresent = true;
        if (null!=document) {

            try {
                Elements elements = document.getElementsByClass(STARTPAGE_RESULTS_LIST_CLASS);
                if (elements.size() == 0) {
                    isLeadPresent = false;
                }
...
//                if(checkIfLeadPresentInStartpage(document))
//                    break;
//                else{
...
//            }
    ...
    public JSONObject getStartpageDataFromDocument(String searchQuery){
        ...
        if(null!=document) {
            try {
                Elements elementList = document.getElementsByClass(STARTPAGE_RESULTS_LIST_CLASS);
