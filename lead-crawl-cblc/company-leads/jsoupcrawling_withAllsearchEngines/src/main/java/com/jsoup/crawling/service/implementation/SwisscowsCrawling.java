...
@Service("com.jsoup.crawling.service.implementation.SwisscowsCrawling")
@Slf4j
public class SwisscowsCrawling {
    @Autowired
    ...
    private boolean checkIfLeadPresentInSwisscows(Document document) {
        boolean isLeadPresent = true;
        if (null!=document) {

            try {
                Elements elements = document.getElementsByClass(SWISSCOWS_RESULTS_LIST_CLASS);
                if (elements.size() == 0) {
                    isLeadPresent = false;
                }
    ...
    private Document getSearchedDocument(String personaUrl, String referrer, String userAgent){
        ...
        try {
            ...
            Connection con = Jsoup.connect(personaUrl).referrer(referrer).userAgent(userAgent).proxy("p.webshare.io",80).timeout(60000);
            document = con.get();
            log.info("document text: "+document.text());
//            for(int trial=0;trial<1;trial++){
//                TimeUnit.SECONDS.sleep(new Random().nextInt(8));
//                if(checkIfLeadPresentInSwisscows(document))
//                    break;
//                else{
...
//            }
    ...
    public JSONObject getSwisscowsDataFromDocument(String searchQuery){
        ...
        if(null!=document) {
            try {
                Elements elementList = document.getElementsByClass(SWISSCOWS_RESULTS_LIST_CLASS);
                if (CollectionUtils.isNotEmpty(elementList)) {
                    for (Element element : elementList) {
                        ...
                        String profileUrl = "";
                        try {
                            searchTitle = element.selectFirst(H2).text();
